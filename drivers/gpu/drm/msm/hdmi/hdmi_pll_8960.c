...
#define to_hdmi_pll_8960(x) container_of(x, struct hdmi_pll_8960, base)
...
static inline void pll_write(struct hdmi_pll_8960 *pll_8960, u32 reg, u32 data)
{
	msm_writel(data, pll_8960->mmio + reg);
}

static inline u32 pll_read(struct hdmi_pll_8960 *pll_8960, u32 reg)
{
	return msm_readl(pll_8960->mmio + reg);
}

static int hdmi_pll_enable(struct clk_hw *hw)
{
	struct hdmi_pll *pll = hw_clk_to_pll(hw);
	struct hdmi_pll_8960 *pll_8960 = to_hdmi_pll_8960(pll);
	...
	pll_write(pll_8960, REG_HDMI_8960_PHY_PLL_LOCKDET_CFG2, 0x8d);
	pll_write(pll_8960, REG_HDMI_8960_PHY_PLL_LOCKDET_CFG0, 0x10);
	pll_write(pll_8960, REG_HDMI_8960_PHY_PLL_LOCKDET_CFG1, 0x1a);
	...
	pll_write(pll_8960, REG_HDMI_8960_PHY_PLL_LOCKDET_CFG2, 0x0d);

	val = pll_read(pll_8960, REG_HDMI_8960_PHY_REG12);
	val |= HDMI_8960_PHY_REG12_SW_RESET;
	/* Assert PHY S/W reset */
	pll_write(pll_8960, REG_HDMI_8960_PHY_REG12, val);
	val &= ~HDMI_8960_PHY_REG12_SW_RESET;
	/* Wait for a short time before de-asserting
	   to allow the hardware to complete its job.
	   This much of delay should be fine for hardware
	...
	pll_write(pll_8960, REG_HDMI_8960_PHY_REG12, val);
	pll_write(pll_8960, REG_HDMI_8960_PHY_REG2,  0x3f);
	...
	val = pll_read(pll_8960, REG_HDMI_8960_PHY_PLL_PWRDN_B);
	val |= HDMI_8960_PHY_PLL_PWRDN_B_PLL_PWRDN_B;
	val &= ~HDMI_8960_PHY_PLL_PWRDN_B_PD_PLL;
	pll_write(pll_8960, REG_HDMI_8960_PHY_PLL_PWRDN_B, val);
	pll_write(pll_8960, REG_HDMI_8960_PHY_REG2, 0x80);

	timeout_count = 1000;
	while (--pll_lock_retry > 0) {

		/* are we there yet? */
		val = pll_read(pll_8960, REG_HDMI_8960_PHY_PLL_STATUS0);
		...
		unsigned long parent_rate)
{
	...
	for (i = 0; pll_rate->conf[i].reg; i++)
		pll_write(pll_8960, pll_rate->conf[i].reg, pll_rate->conf[i].val);

	pll_8960->pixclk = rate;
...
struct hdmi_pll *hdmi_pll_8960_init(struct platform_device *pdev)
{
	...
	pll_8960 = devm_kzalloc(dev, sizeof(*pll_8960), GFP_KERNEL);
	if (!pll_8960)
		return ERR_PTR(-ENOMEM);

	pll_8960->mmio = msm_ioremap(pdev, "hdmi_pll", "HDMI_PLL");
	if (IS_ERR(pll_8960->mmio)) {
