...
static int arm_smmu_init_clocks(struct arm_smmu_device *smmu)
{
	...
	of_property_for_each_string(dev->of_node, "clock-names", prop, cname) {
		...
		/*
		 * clks are prepared early as the runtime callbacks to enable
		 * the clocks can be called from the master in atomic contexts.
		 */
		ret = clk_prepare(c);
		if (ret) {
			dev_err(dev, "Couldn't clk_prepare for %s", cname);
			return ret;
		}
